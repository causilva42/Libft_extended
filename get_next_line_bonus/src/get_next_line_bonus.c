...
/*   Updated: 2025/05/20 12:03:23 by causilva         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	buffer[BUFFER_MAXFD + 1][BUFFER_SIZE + 1];
	t_dynstr	line;
	int			read_return;
	...
	while (read_return != 0)
	{
		...
		update_string(&line, buffer[fd]);
		if (!line.string)
			return (NULL);
		read_return *= update_buffer(buffer[fd]);
	}
	if (line.string[0] == '\0')
		return (free(line.string), NULL);
	return (line.string);
